...
class ObservableObject {
        ...
        _private.set(this, {
            ...
            setProp: function(inPath, inValue, inBackPath, inAlreadyFoundChange) {
                ...
                if (!path.length) {
                    ...
                    let oldValueWasObservableObject = oldValue instanceof ObservableObject;
                    if (oldValue === undefined || oldValue === null || !oldValueWasObservableObject) {
                        oldValue = new ObservableObject();
                        _p.props.prop(localProp, oldValue);
                        _p.changesQueue.push({
                            path: inBackPath.concat(path).join('.'),
                            change: {
                                type: !oldValueWasObservableObject ? 'replace' : 'add',
                                oldValue: undefined,
                                newValue: _p.props.prop(localProp)
                            }
    ...
    * [Symbol.iterator]() {
    ...
    get observer() {
        return _private.get(this).observer;
    }
